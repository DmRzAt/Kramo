(() => {
	...
	const setupVariationSwatches = (form) => {
		...
		form.querySelectorAll(".variations select").forEach((select) => {
			if (select.dataset.woostarterSwatches === "ready") {
				return;
			}

			select.dataset.woostarterSwatches = "ready";
			select.classList.add("woostarter-native-select");

			const wrapper = document.createElement("div");
			const isColor = /kolor|color/i.test(select.name);
			wrapper.className = `woostarter-variation-options ${
				isColor
					? "woostarter-variation-options--color"
					: "woostarter-variation-options--size"
			}`;
			...
			const rebuild = () => {
				...
				Array.from(select.options).slice(1).forEach((option) => {
					...
					button.type = "button";
					button.className = "woostarter-variation-option";
					...
					if (isColor) {
						button.classList.add(
							"woostarter-variation-option--color",
							`woostarter-swatch--${slugify(option.textContent)}`
						);
	...
	const setupProductTabs = () => {
		...
		if (!tabs || tabs.dataset.woostarterTabs === "ready") {
			return;
		}

		tabs.dataset.woostarterTabs = "ready";
		...
		panels.forEach((panel, index) => {
			panel.dataset.woostarterDesktopDisplay = panel.style.display;
			...
			const button = document.createElement("button");
			button.type = "button";
			button.className = "woostarter-accordion-toggle";
		...
		const syncLayout = () => {
			const buttons = Array.from(tabs.querySelectorAll(".woostarter-accordion-toggle"));
			...
			if (mobile.matches) {
				...
				panels.forEach((panel) => {
					panel.hidden = false;
					panel.style.display = panel.dataset.woostarterDesktopDisplay;
				});
			}
		};
	...
	document.addEventListener("click", (event) => {
		const trigger = event.target.closest(".woostarter-video-trigger");
		if (!trigger) {
			return;
		}
		...
		media.className = "woostarter-product-video";
