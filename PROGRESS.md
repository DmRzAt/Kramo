...
| 11 — Security & backups | Виконано | Заголовки, XML-RPC 403, RODO-банер із реальним блокуванням, тест відновлення пройдено |
| 12 — Client handoff | Виконано | `instrukcja-klienta.md` (9 розділів), `make-manual.sh`, `handoff-checklist.md` |

**Усі 12 завдань плану виконано.**
...
## Результат завдання 11 (безпека і бекапи)

**Безпека** — без важких плагінів, усе в `inc/security.php`. Перевірено запитами:

| Перевірка | Результат |
|---|---|
| `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Opener-Policy` | усі віддаються |
| `Strict-Transport-Security` | вмикається на HTTPS |
| Версія WP у `<head>` і в `?ver=` | прибрано |
| `xmlrpc.php` | 403 на GET і POST |
| `?author=1` | 301 на головну |
| `/wp-json/wp/v2/users` анонімно | `rest_no_route` |
...
**Тест відновлення пройдено** (вимога завдання):

1. Повна копія: база 91,6 KB, плагіни 38,2 MB, теми 14,2 MB, uploads 426 KB.
2. Видалено товар «Czapka bawełniana» з 4 варіантами → 12 → 11 товарів.
3. Відновлено базу з архіву UpdraftPlus.
4. Результат: 12 товарів, товар із 4 варіантами повернувся, усі сторінки 200, налаштування (пресет, зона доставки, WebP, розклад копій) не постраждали.

Документи: `docs/bezpieczenstwo-kopie.md`, `docs/polityka-prywatnosci.md`, `docs/regulamin.md` (шаблони — клієнт перевіряє з юристом).
...
PDF локально не згенеровано: pandoc не встановлений на машині. Скрипт перевірено — синтаксис чистий, шлях без pandoc дає зрозумілу помилку й код 1.
