...
function kramo_local_service_fields() {
	return array(
		'_kramo_ls_service'  => __( 'Usługa', 'kramo' ),
		'_kramo_ls_city'     => __( 'Miasto', 'kramo' ),
		'_kramo_ls_area'     => __( 'Obszar obsługi (miasta po przecinku)', 'kramo' ),
		'_kramo_ls_price'    => __( 'Cena / przedział (np. 20–40 zł/m²)', 'kramo' ),
		'_kramo_ls_phone'    => __( 'Telefon', 'kramo' ),
		'_kramo_ls_cta_text' => __( 'Tekst przycisku CTA', 'kramo' ),
		'_kramo_ls_cta_url'  => __( 'Link przycisku CTA', 'kramo' ),
	);
}
...
function kramo_render_local_service_metabox( $post ) {
	...
	foreach ( kramo_local_service_fields() as $key => $label ) {
		$value = (string) kramo_post_meta_value( $post->ID, $key );
		printf(
	...
	$faq = (string) kramo_post_meta_value( $post->ID, '_kramo_ls_faq' );
	printf(
		'<p><label for="_kramo_ls_faq"><strong>%1$s</strong></label><br><span class="description">%2$s</span><br><textarea id="_kramo_ls_faq" name="_kramo_ls_faq" rows="6" class="widefat">%3$s</textarea></p>',
...
function kramo_save_local_service_metabox( $post_id ) {
	...
	foreach ( array_keys( kramo_local_service_fields() ) as $key ) {
		...
		if ( '_kramo_ls_cta_url' === $key ) {
			$value = esc_url_raw( $value );
		}
		update_post_meta( $post_id, $key, $value );
	}

	$faq = isset( $_POST['_kramo_ls_faq'] ) ? sanitize_textarea_field( wp_unslash( $_POST['_kramo_ls_faq'] ) ) : '';
	update_post_meta( $post_id, '_kramo_ls_faq', $faq );
}
...
function kramo_get_local_service_data( $post_id ) {
	$area_raw = (string) kramo_post_meta_value( $post_id, '_kramo_ls_area' );
	$area     = array_values(
		array_filter( array_map( 'trim', explode( ',', $area_raw ) ) )
	);

	return array(
		'service'     => (string) kramo_post_meta_value( $post_id, '_kramo_ls_service' ),
		'city'        => (string) kramo_post_meta_value( $post_id, '_kramo_ls_city' ),
		'area_served' => $area,
		'price_range' => (string) kramo_post_meta_value( $post_id, '_kramo_ls_price' ),
		'phone'       => (string) kramo_post_meta_value( $post_id, '_kramo_ls_phone' ),
		'faq_raw'     => (string) kramo_post_meta_value( $post_id, '_kramo_ls_faq' ),
		'cta_text'    => (string) kramo_post_meta_value( $post_id, '_kramo_ls_cta_text' ),
		'cta_url'     => (string) kramo_post_meta_value( $post_id, '_kramo_ls_cta_url' ),
	);
}
