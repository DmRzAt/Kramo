 ...
 */
function woostarter_critical_style_handles() {
	return array( 'woostarter-tokens', 'woostarter-base', 'woostarter-preset' );
}
 ...
 * @return bool
 */
function woostarter_woocommerce_css_is_critical() {
 ...
 * @return string
 */
function woostarter_read_theme_asset( $relative_path ) {
 ...
 * @return string
 */
function woostarter_rebase_css_urls( $css, $relative_path ) {
 ...
 */
function woostarter_inline_critical_css() {
	$styles = wp_styles();
	if ( ! $styles ) {
		return;
	}
	...
	$paths = array(
		'woostarter-tokens'  => 'assets/css/tokens.css',
		'woostarter-base'    => 'assets/css/base.css',
		'woostarter-preset'  => woostarter_get_active_preset_stylesheet(),
	);

	$inlined = '';
	foreach ( woostarter_critical_style_handles() as $handle ) {
		if ( ! isset( $styles->registered[ $handle ] ) || empty( $paths[ $handle ] ) ) {
			continue;
		}

		$css = woostarter_read_theme_asset( $paths[ $handle ] );
		if ( '' === $css ) {
			continue;
		}

		$inlined .= woostarter_rebase_css_urls( $css, $paths[ $handle ] );
		$GLOBALS['woostarter_inlined_handles'][] = $handle;
	}
	...
	wp_register_style( 'woostarter-critical', false ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
	wp_enqueue_style( 'woostarter-critical' );
	wp_add_inline_style( 'woostarter-critical', $inlined );
}
add_action( 'wp_enqueue_scripts', 'woostarter_inline_critical_css', 100 );
 ...
 * @return string
 */
function woostarter_suppress_inlined_style_tags( $html, $handle ) {
	if ( is_admin() ) {
		return $html;
	}

	$inlined = isset( $GLOBALS['woostarter_inlined_handles'] )
		? (array) $GLOBALS['woostarter_inlined_handles']
		: array();

	return in_array( $handle, $inlined, true ) ? '' : $html;
}
add_filter( 'style_loader_tag', 'woostarter_suppress_inlined_style_tags', 15, 2 );
 ...
 * @return string
 */
function woostarter_async_noncritical_styles( $html, $handle ) {
	if ( is_admin() || 'woostarter-woocommerce' !== $handle ) {
		return $html;
	}

	if ( woostarter_woocommerce_css_is_critical() ) {
		return $html;
	}
...
add_filter( 'style_loader_tag', 'woostarter_async_noncritical_styles', 20, 2 );
 ...
 * @return string
 */
function woostarter_prioritize_product_image( $html, $thumbnail_id = 0 ) {
	unset( $thumbnail_id );
...
add_filter( 'woocommerce_single_product_image_thumbnail_html', 'woostarter_prioritize_product_image', 10, 2 );
 ...
 */
function woostarter_image_dimension_attributes( $attr ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'woostarter_image_dimension_attributes' );
 ...
 */
function woostarter_allow_priority_attributes( $tags, $context ) {
...
add_filter( 'wp_kses_allowed_html', 'woostarter_allow_priority_attributes', 10, 2 );
 ...
 */
function woostarter_trim_payment_assets() {
	if ( is_admin() || ! function_exists( 'is_cart' ) ) {
		return;
	}

	$payment_screen = is_cart() || is_checkout() || is_account_page();
	if ( apply_filters( 'woostarter_load_payment_assets', $payment_screen ) ) {
		return;
	}
...
add_action( 'wp_enqueue_scripts', 'woostarter_trim_payment_assets', 999 );
 ...
 * @return int
 */
function woostarter_omit_loading_attr_threshold() {
	return 4;
}
add_filter( 'wp_omit_loading_attr_threshold', 'woostarter_omit_loading_attr_threshold' );
