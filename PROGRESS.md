...
Розгорнуто на Railway: **https://kramo-production.up.railway.app**

`Dockerfile.production` + `railway.json` + `deployment/production-entrypoint.sh`. Перший старт сам ставить WordPress, тему, плагіни, польські налаштування, сторінки, зону доставки, Rank Math і демо-контент; health `/kramo-health` зеленіє лише після успішного провізіонінгу.

Перевірено на живому домені: усі сторінки (головна, каталог, товар, кошик, checkout, конто, локальна сторінка, sitemap) — 200; 12 товарів, зображення WebP, критичний CSS інлайном, демо-стрічка, усі 5 заголовків безпеки + HSTS.

**Lighthouse mobile на реальному хостингу (сторінка товару): 97, LCP 1,9 s, CLS 0, TBT 0 ms, відповідь сервера 350 ms.** Це закриває останню відкриту ціль завдання 10 — локально LCP не влазив у 2,0 s саме через відсутність кешу сторінок, а не через код.

## Дизайн-система (2026-07-26)

Проведено через gstack `/design-consultation` із дослідженням ринку (rzeczysame.pl, pakamera.pl). Результат — `DESIGN.md` як джерело правди + `CLAUDE.md` правило звірятися з ним.

Знайдена дірка ринку: один конкурент холодний і акуратний, другий теплий і хаотичний. Ніхто не дає тепло + спокій + довіру разом. Теза: «видно, що зроблено руками — і видно, що куплю без ризику».

- **Палітра:** тепла основа `#F7F5F2`, чорнило `#1C1A17`, єдиний акцент амарант `#A32D4F` (польський геральдичний, 6.90:1 на білому), хвоя `#2F4F3F` для іконок довіри.
- **Типографіка:** Bricolage Grotesque (заголовки) + Instrument Sans (текст), обидва SIL OFL, самохостинг, subset під польську — 36K + 28K.
- **Смуга довіри** під хедером: BLIK, Paczkomat InPost + Orlen, поріг безкоштовної доставки (читається з реальної зони), 14 днів на зворот, польська праковня. Конкуренти тримають це в підвалі.
- **Рух:** scroll-reveal зі stagger, підйом картки, зум фото 1.04, тінь хедера при скролі — усе під `prefers-reduced-motion`.

**Ліцензійна пастка, яку зловили:** спершу обрані Cabinet Grotesk + General Sans (Fontshare) були завантажені й перевірені, але їхній EULA §02 забороняє модифікацію (subset) і викладання файлів на публічний сервер — тобто ні наш GitHub, ні передача клієнту. Замінено на OFL-аналоги, які це дозволяють.

Lighthouse після зміни (локально, mobile, сторінка товару): **95**, CLS 0, TBT 10 ms — вище, ніж було до дизайну (91).

Демо-режим: `KRAMO_DEMO=1` вмикає стрічку й глушить вихідну пошту, оплата — Cash on delivery, тож checkout проходиться до кінця без бойових ключів. Пресет міняється змінною `KRAMO_PRESET` без перезбірки.
