 ...
 */
function woostarter_shipping_config() {
	return apply_filters(
		'woostarter_shipping_config',
		array(
 ...
 */
function woostarter_classify_shipping_rate( $rate ) {
 ...
 * @return float
 */
function woostarter_package_weight( $package ) {
	$weight = 0.0;
 ...
 * @return float
 */
function woostarter_profile_cost( $profile, $weight ) {
 ...
 */
function woostarter_adjust_package_rates( $rates, $package ) {
	$config = woostarter_shipping_config();
	$weight = woostarter_package_weight( $package );

	foreach ( $rates as $rate ) {
		$profile_key = woostarter_classify_shipping_rate( $rate );
		if ( null === $profile_key ) {
			continue;
		}

		$cost = woostarter_profile_cost( $config['profiles'][ $profile_key ], $weight );
		$rate->set_cost( $cost );
	...
	if ( $subtotal >= $config['free_threshold'] ) {
		...
		if ( $has_free ) {
			foreach ( $rates as $key => $rate ) {
				if ( null !== woostarter_classify_shipping_rate( $rate ) ) {
					unset( $rates[ $key ] );
				}
			}
		}
...
add_filter( 'woocommerce_package_rates', 'woostarter_adjust_package_rates', 20, 2 );
 ...
 */
function woostarter_pickup_point_meta_keys() {
	return apply_filters(
		'woostarter_pickup_point_meta_keys',
		array(
 ...
 * @return string
 */
function woostarter_get_order_pickup_point( $order ) {
	foreach ( woostarter_pickup_point_meta_keys() as $key ) {
 ...
 */
function woostarter_store_pickup_point( $order ) {
	if ( '' !== woostarter_get_order_pickup_point( $order ) ) {
		return;
	}
...
add_action( 'woocommerce_checkout_create_order', 'woostarter_store_pickup_point', 20 );
 ...
 * @param WC_Order $order Order object.
 */
function woostarter_admin_pickup_point( $order ) {
	$point = woostarter_get_order_pickup_point( $order );
	if ( '' === $point ) {
		return;
	}
	...
	printf(
		...
		esc_html__( 'Punkt odbioru:', 'woostarter' ),
		esc_html( $point )
	);
}
add_action( 'woocommerce_admin_order_data_after_shipping_address', 'woostarter_admin_pickup_point' );
 ...
 * @param WC_Order $order Order object.
 */
function woostarter_email_pickup_point( $order ) {
	$point = woostarter_get_order_pickup_point( $order );
	if ( '' === $point ) {
		return;
	}
...
add_action( 'woocommerce_email_after_order_table', 'woostarter_email_pickup_point', 15 );
add_action( 'woocommerce_order_details_after_order_table', 'woostarter_email_pickup_point', 15 );
